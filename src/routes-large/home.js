...
class Home extends Component {
  ...
  onRefresh = () => {
    // 刷新数据 
    this.setState({loading: true, hasMore: true, refreshing: true, isLoading: true, page: 1, songList: [], selectedRowID: -1 },()=>{
      this.getList();
    });    
  }
  ...
  onEndReached = () => {
     ...
     this.setState({ loading: true, isLoading: true, page: page + 1 }, ()=>{
      this.getList();
    });   
  }
  ...
  onClickBtns = (flag) => { //按钮组
    ...
    switch(flag){
      ...
      case 2: //点单
         if (window.android){
            window.android.clickOrderDrink();
         } else{
            path = "/drinks-large"
          }
        break;
  ...
  render() {
    const {refreshing, isLoading, dataSource, size, bangdanList, openCtr, loading } = this.state
